...
function Location({match}) {
    ...
    useEffect(() => {
        const fetchLocation = async () => {
            const fetchLocation = await fetch(`https://ghibliapi.herokuapp.com/locations/${match.params.id}`);
    ...
    if (Array.isArray(locationDetail.films) && locationDetail.films.length) {
        relatedFilms = locationDetail.films.map((url, index) => <RelatedFilms key={index} urlFilm={url} />);
    };
